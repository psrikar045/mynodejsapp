...
async function scrapeLinkedInCompany(url, browser, linkedinAntiBot = null) {
  ...
  try {
    ...
    await interactionHandler.startObserver(); // Start observing for delayed pop-ups
    console.log('[+] Adaptive Interaction Handler is active.');

    // Give a moment for the initial handlers to work
    await delay(2000);
    ...
    try {
      // **NEW: Human-like closing behavior**
      console.log('Simulating human-like closing behavior...');
      await antiBotSystem.humanDelay(500, 1500); // Wait for a bit
      await antiBotSystem.simulateHumanBehavior(page, {
          enableMouseMovement: true,
          enableScrolling: false // Don't scroll before closing
      });
      await page.close();
      console.log(`✅ Page closed for ${url} with human-like delay.`);
    } catch (closeError) {
